...
async def ai_generate_transform(
    ...
    # Auto-detect date-like columns from sample data to help the LLM
    import re as _re
    date_patterns = [
        r'\d{4}[-/]\d{1,2}[-/]\d{1,2}',
        r'\d{1,2}[-/]\d{1,2}[-/]\d{4}',
    ...
    detected_date_cols = []
    for col in headers:
        col_values = [str(row.get(col, '')) for row in sample_rows[:10] if row.get(col)]
        date_hit = sum(
            1 for v in col_values
            if any(_re.search(p, v) for p in date_patterns)
        )
        if date_hit >= max(1, len(col_values) * 0.5):
...
- For date formatting: ALWAYS use pd.to_datetime(df[col], infer_datetime_format=True, errors='coerce') then .dt.strftime(fmt)
- If user says "all date columns" or "date columns": apply the transformation to EVERY date column listed, not just one
- Generate a LOOP or apply the code to EACH date column separately
- After date conversion always chain .fillna('') to handle NaT values
- Columns_affected must list ALL columns the code touches

Respond ONLY with valid JSON:
{
  "pandas_code": "for col in ['col1', 'col2']: df[col] = pd.to_datetime(df[col], infer_datetime_format=True, errors='coerce').dt.strftime('%d/%m/%Y').fillna('')",
...
Detected date-like columns (based on sample values — apply date transforms to ALL of these):
{detected_date_cols if detected_date_cols else "None detected — use column names and sample values to identify"}
...
IMPORTANT: If the instruction mentions dates or date formatting:
1. Apply to ALL {len(detected_date_cols)} detected date columns: {detected_date_cols}
2. Use a loop: for col in {detected_date_cols}: df[col] = pd.to_datetime(...)
3. Use format='mixed' or infer_datetime_format=True to handle mixed date formats
4. Always add .fillna('') after strftime
